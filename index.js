...
app.get('/api/mux-playback-by-asset/:assetId', async (req, res) => {
  try {
    let asset = await mux.assets.retrieve(req.params.assetId);
    let playbackId = asset.playback_ids?.[0]?.id;

    // If asset has no playback id, create a public one now
    if (!playbackId) {
      try {
        const created = await mux.assets.playbackIds.create(asset.id, { policy: 'public' });
        playbackId = created.id;
        // refresh asset info (optional)
        asset = await mux.assets.retrieve(asset.id);
      } catch (e) {
...
app.get('/api/mux-upload-status/:uploadId', async (req, res) => {
  try {
    const upload = await mux.uploads.retrieve(req.params.uploadId);
    res.json(upload);
  } catch (error) {
    const code = error?.status || error?.statusCode || 500;
...
app.get('/api/mux-playback/:id', async (req, res) => {
  ...
  try {
    ...
    try {
      const upload = await mux.uploads.retrieve(id);
      const assetId = upload?.asset_id;
      if (assetId) {
        const asset = await mux.assets.retrieve(assetId);
        let playbackId = asset.playback_ids?.[0]?.id;
        if (!playbackId) {
          try {
            const created = await mux.assets.playbackIds.create(asset.id, { policy: 'public' });
    ...
    try {
      const asset = await mux.assets.retrieve(id);
      let playbackId = asset.playback_ids?.[0]?.id;
      if (!playbackId) {
        try {
          const created = await mux.assets.playbackIds.create(asset.id, { policy: 'public' });
...
app.get('/api/mux-signed-playback-by-asset/:assetId', async (req, res) => {
  try {
    ...
    const asset = await mux.assets.retrieve(req.params.assetId);
    let playbackId = asset.playback_ids?.[0]?.id;
    if (!playbackId) {
      const created = await mux.assets.playbackIds.create(asset.id, { policy: 'signed' });
      playbackId = created.id;
    }
...
app.delete('/api/mux-asset/:assetId', requireAdminPassword, async (req, res) => {
  try {
    ...
    await mux.assets.delete(assetId);
...
app.post('/api/mux-asset/delete', async (req, res) => {
  ...
  try {
    ...
    if (!assetId) {
      ...
      if (playbackId) {
        try {
          const playback = await mux.playbackIds.retrieve(playbackId);
